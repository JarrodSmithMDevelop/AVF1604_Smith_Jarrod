...
//Rare exceptions to indicate a bug in the _all_things_weather code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_all_things_weather = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_all_things_weather]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.all_things_weather.module"
