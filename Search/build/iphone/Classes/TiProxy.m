...
//Rare exceptions to indicate a bug in the _memories code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_memories = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_memories]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.memories.module"
